% Two-body Active Inference Simulation (Both Agents Enabled)
...
x1=zeros(1,T); x2=zeros(1,T);
z1=zeros(1,T); z2=zeros(1,T);
v1=zeros(1,T); v2=zeros(1,T);
zv1=zeros(1,T); zv2=zeros(1,T);
a1=zeros(1,T); a2=zeros(1,T);
za1=zeros(1,T); za2=zeros(1,T);
mu_x1=zeros(1,T); mu_x2=zeros(1,T);
mu_v1=zeros(1,T); mu_v2=zeros(1,T);
mu_a1=zeros(1,T); mu_a2=zeros(1,T);
y_x1=zeros(1,T); y_x2=zeros(1,T);
y_v1=zeros(1,T); y_v2=zeros(1,T);
grad_mu1=zeros(3,T); grad_mu2=zeros(3,T);
grad_v1=zeros(1,T); grad_zv1=zeros(1,T);
grad_v2=zeros(1,T); grad_zv2=zeros(1,T);
f1=zeros(1,T); f2=zeros(1,T);

PI_x=init_PI_tilde(3,L,[0.5],[1]);
PI_y=init_PI_tilde(2,L,[0.5],[1]);
eta_x=5;
...
v2(1)=0; v1(1)=0;
zv2(1)=1;zv1(1)=-1;
mu_x1(1)=eta_x; mu_x2(1)=eta_x;

ey_tilde_1=zeros(2,1,T); ex_tilde_1=zeros(3,1,T);

ey_tilde_2=zeros(2,1,T); ex_tilde_2=zeros(3,1,T);
...
for t=2:T
    % Dynamics update
    v1(t)=v1(t-1) + dt*a1(t-1); v2(t)=v2(t-1) + dt*a2(t-1);
    zv1(t)=zv1(t-1) + dt*za1(t-1); zv2(t)=zv2(t-1) + dt*za2(t-1);

    x1(t)=x1(t-1) + dt*v1(t)/(eps+sqrt(v1(t).^2 + zv1(t)^2)); x2(t)=x2(t-1) + dt*v2(t)/(eps+sqrt(v2(t).^2 + zv2(t)^2));
    z1(t)=z1(t-1) + dt*zv1(t)/(eps+sqrt(v1(t).^2 + zv1(t)^2)); z2(t)=z2(t-1) + dt*zv2(t)/(eps+sqrt(v2(t).^2 + zv2(t)^2));

    % Sensory inputs
    dist = sqrt((x2(t)-x1(t))^2 + (z2(t)-z1(t))^2);
    dotprod = (x1(t)-x2(t))*(v1(t)-v2(t)) + (z1(t)-z2(t))*(zv1(t)-zv2(t));
    y_x1(t)=dist; y_x2(t)=dist;
    y_v1(t)=dotprod / dist; y_v2(t)=y_v1(t);

    % Prediction errors
    ey_tilde_1(:,:,t) = [y_x1(t) - mu_x1(t-1); y_v1(t) - mu_v1(t-1)];
    ex_tilde_1(:,:,t) = [mu_v1(t-1) + alpha*(mu_x1(t-1)-eta_x);
                         mu_a1(t-1) + alpha*(mu_v1(t-1));
                         alpha*mu_a1(t-1)];

    ey_tilde_2(:,:,t) = [y_x2(t) - mu_x2(t-1); y_v2(t) - mu_v2(t-1)];
    ex_tilde_2(:,:,t) = [mu_v2(t-1) + alpha*(mu_x2(t-1)-eta_x);
                         mu_a2(t-1) + alpha*(mu_v2(t-1));
                         alpha*mu_a2(t-1)];

    % Free energy
    f1(t)=ey_tilde_1(:,:,t)'*PI_y*ey_tilde_1(:,:,t) + ex_tilde_1(:,:,t)'*PI_x*ex_tilde_1(:,:,t);
    f2(t)=ey_tilde_2(:,:,t)'*PI_y*ey_tilde_2(:,:,t) + ex_tilde_2(:,:,t)'*PI_x*ex_tilde_2(:,:,t);
    ...
    if t > to_mu
        ...
        mu_x2(t)=mu_x2(t-1)+k_mu*(mu_v2(t-1)-grad_mu2(1,t));
        mu_v2(t)=mu_v2(t-1)+k_mu*(mu_a2(t-1)-grad_mu2(2,t));
        mu_a2(t)=mu_a2(t-1)+k_mu*(0-grad_mu2(3,t));
    else
        mu_x1(t)=mu_x1(t-1); mu_v1(t)=mu_v1(t-1); mu_a1(t)=mu_a1(t-1);
        mu_x2(t)=mu_x2(t-1); mu_v2(t)=mu_v2(t-1); mu_a2(t)=mu_a2(t-1);
    end

    grad1 = grad_Act(mu_v1(t),v1(t),v2(t),x1(t),x2(t),z1(t),z2(t),zv1(t),zv2(t));
    grad2 = grad_Act(mu_v2(t),v2(t),v1(t),x2(t),x1(t),z2(t),z1(t),zv2(t),zv1(t));

    grad_v1(t) = grad1(1); grad_zv1(t) = grad1(2);
    grad_v2(t) = grad2(1); grad_zv2(t) = grad2(2);

    a1(t) = (t > to_act)*(-k_a*grad_v1(t));
    za1(t) = (t > to_act)*(-k_a*grad_zv1(t));
    a2(t) = (t > to_act)*(-k_a*grad_v2(t));
    za2(t) = (t > to_act)*(-k_a*grad_zv2(t));
end
...
subplot(3,2,5)
plot(time, grad_v1, 'color',col1); hold on;
plot(time, grad_zv1,'color',col2); % Uncomment for Agent 2
...
function ret=init_PI_tilde(o,L,gamma_vecdL,lambda_vecdL)
    ...
    PI=cell(1,dL);
    parfor j=1:dL
        Spatial=inv(gamma_vecdL(j)*eye(L));
        Temporal=spm_DEM_R(o,lambda_vecdL(j));
        PI{j}=kron(Temporal,Spatial);
    end
    ret=PI{1};
end
